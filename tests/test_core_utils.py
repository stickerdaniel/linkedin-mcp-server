...
class TestScrollToBottomPacing:
    async def test_the_pause_between_scrolls_is_jittered(self):
        ...
        with (
            ...
            # Two rounds of heights are mocked, and one stale round no longer
            # ends the loop, so the bound is what stops it.
            await scroll_to_bottom(page, pause_time=1.0, max_scrolls=2)
