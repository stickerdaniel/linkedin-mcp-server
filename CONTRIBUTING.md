...
A PR titled `feat`, `fix` or with a `!` needs a changelog fragment. The PR
Title check fails until it is there.

1. Open the PR as a draft to get its number.
2. Add `changelog.d/<number>.feat.md`, `.fix.md` or `.breaking.md` to match the
   title. A `!` title takes `.breaking.md` whatever its type.
...
Retitling to or from `!` means renaming the fragment. Edit an existing fragment
instead of running `towncrier create` again, which writes a second, numbered
file that the check rejects.
