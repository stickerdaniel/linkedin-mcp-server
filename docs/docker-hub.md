...
| `BROWSER_MIN_HOLD` | `25` | Shortest time (seconds) a process keeps the shared browser before handing it to a waiting process. Higher means fewer browser restarts but longer waits for other clients. `0` hands over after every tool call. |
