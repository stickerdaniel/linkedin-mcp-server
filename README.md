...
- *Windows, `DLL load failed while importing _greenlet`*: usually a missing [Microsoft Visual C++ Redistributable](https://learn.microsoft.com/en-us/cpp/windows/latest-supported-vc-redist). greenlet 3.3.1 through 3.5.4 need `MSVCP140.dll` from it, and no Python distribution ships that DLL. Without administrator rights, `uvx --with "greenlet<=3.3.0" mcp-server-linkedin` works instead on x86-64; the older greenlet has no Windows ARM64 wheel. If the redistributable is already installed, the loader is reporting a different problem and the full error names it. Tracked upstream as [greenlet#525](https://github.com/python-greenlet/greenlet/issues/525).
...
- *Windows, the bundle exits with `DLL load failed while importing _greenlet`*: usually a missing [Microsoft Visual C++ Redistributable](https://learn.microsoft.com/en-us/cpp/windows/latest-supported-vc-redist). greenlet 3.3.1 through 3.5.4 need `MSVCP140.dll` from it, and no Python distribution ships that DLL. If it is already installed, the loader is reporting a different problem and the full error names it. Tracked upstream as [greenlet#525](https://github.com/python-greenlet/greenlet/issues/525).
