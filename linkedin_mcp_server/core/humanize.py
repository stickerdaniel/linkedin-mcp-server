...
async def human_type(
    ...
    *,
    ...
    A uniform per-key delay (a fixed 15ms, say) is itself a tell; this removes
    it. For any field the scraper types into. The message composer is not one:
    it writes its text atomically through ``insertText`` inside a pinned
    editor and never goes through the keyboard.
