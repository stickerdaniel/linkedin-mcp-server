...
class PersonExtended(Person):
    ...
    def extract_contact_info(self):
        ...
        if self.linkedin_url not in driver.current_url:
        ...
        try:
            ...
            driver.execute_script(
                "arguments[0].scrollIntoView(true);", contact_info_link
            )
            time.sleep(1)
    ...
    def _extract_modal_data(self, driver):
        ...
        try:
            ...
            logger.debug(
                f"Modal text preview: {modal_text[:200] if modal_text else 'No text found'}..."
            )

            # Extract LinkedIn Profile URL
            try:
                # Try multiple approaches
                profile_links = modal.find_elements(
                    By.XPATH, ".//a[contains(@href, 'linkedin.com/in/')]"
                )
                if profile_links:
                    self.contact_info["linkedin_url"] = profile_links[0].get_attribute(
                        "href"
                    )
                    logger.info(
                        f"Found LinkedIn URL: {self.contact_info['linkedin_url']}"
                    )
            except Exception as e:
            ...
            # Extract Websites
            try:
                websites_section = modal.find_element(
                    By.XPATH, ".//section[contains(., 'Websites')]"
                )
                ...
                for selector in phone_selectors:
                    try:
                        ...
                        text = (
                            ...
                            else phone_elem.get_attribute("href").replace("tel:", "")
                        )
            ...
            try:
                ...
                for selector in email_selectors:
                    try:
                        ...
                        if email_elem.get_attribute(
                            "href"
                        ) and "mailto:" in email_elem.get_attribute("href"):
                            email = email_elem.get_attribute("href").replace(
                                "mailto:", ""
                            )
                        else:
            ...
            # Extract Birthday
            try:
                birthday_section = modal.find_element(
                    By.XPATH, ".//section[contains(., 'Birthday')]"
                )
                # Birthday is usually in a time element or the text after Birthday label
                birthday_elements = birthday_section.find_elements(
                    By.XPATH, ".//time | .//span[not(contains(text(), 'Birthday'))]"
                )
            ...
            try:
                connected_section = modal.find_element(
                    By.XPATH, ".//section[contains(., 'Connected')]"
                )
                ...
                logger.info(
                    f"Found connected date: {self.contact_info['connected_date']}"
                )
            except NoSuchElementException:
                pass
